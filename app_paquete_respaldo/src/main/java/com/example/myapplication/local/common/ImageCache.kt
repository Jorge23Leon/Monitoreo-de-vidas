...
object ImageCache {
    ...
    private const val MAX_IMAGE_DOWNLOAD_BYTES = 12L * 1024L * 1024L
    private const val MAX_CACHE_BYTES = 100L * 1024L * 1024L
    private const val CACHE_TARGET_BYTES = 80L * 1024L * 1024L
    ...
    suspend fun guardarEnCache(
        ...
        try {
            ...
            destino.setLastModified(System.currentTimeMillis())
            recortarCache(carpeta, proteger = destino)
    ...
    private fun procesarRespuesta(
        ...
        val limiteRespuesta = if (esContenidoJson(contentType)) {
            MAX_JSON_BYTES
        } else {
            MAX_IMAGE_DOWNLOAD_BYTES
        }
        val contentLength = body.contentLength()
        if (contentLength > limiteRespuesta) {
            Log.w(TAG, "RESPUESTA_DEMASIADO_GRANDE url=$urlActual bytes=$contentLength")
            return false
        }
        ...
        if (esContenidoJson(contentType)) {
            val json = runCatching {
                body.byteStream().use { input ->
                    String(leerConLimite(input, MAX_JSON_BYTES), Charsets.UTF_8)
                }
            }
                .getOrElse { error ->
        ...
        body.byteStream().use { entrada ->
            FileOutputStream(temporal).use { salida ->
                copiarConLimite(entrada, salida, MAX_IMAGE_DOWNLOAD_BYTES)
                salida.fd.sync()
            }
        }
        ...
        return try {
            when {
                ruta.startsWith("content://", ignoreCase = true) -> {
                    ...
                    val opciones = opcionesParaLimite(bounds, 2048)
                    context.contentResolver.openInputStream(uri)
                ...
                ruta.startsWith("file://", ignoreCase = true) -> {
                    ...
                    decodificarArchivoLimitado(path, 2048)
                }

                esRutaArchivoAbsoluta(ruta) -> decodificarArchivoLimitado(ruta, 2048)
                else -> null
            }
        } catch (e: Exception) {
    ...
    private fun decodificarArchivoLimitado(path: String, maxDimension: Int): Bitmap? {
        val bounds = BitmapFactory.Options().apply { inJustDecodeBounds = true }
        BitmapFactory.decodeFile(path, bounds)
        return BitmapFactory.decodeFile(path, opcionesParaLimite(bounds, maxDimension))
    }

    private fun opcionesParaLimite(
        bounds: BitmapFactory.Options,
        maxDimension: Int
    ): BitmapFactory.Options {
        var muestra = 1
        while (
            bounds.outWidth / (muestra * 2) > maxDimension ||
            bounds.outHeight / (muestra * 2) > maxDimension
        ) {
            muestra *= 2
        }
    ...
    private fun archivoEsImagenValida(archivo: File): Boolean {
        if (!archivo.exists() || archivo.length() !in 1..MAX_IMAGE_DOWNLOAD_BYTES) return false
    ...
    private fun copiarConLimite(
        ...
        while (true) {
            ...
            if (leidos < 0) break
            total += leidos
            require(total <= limite) { "La imagen supera el límite permitido." }
            salida.write(buffer, 0, leidos)
        }
    }

    private fun leerConLimite(entrada: InputStream, limite: Long): ByteArray {
        ...
        while (true) {
            ...
            if (leidos < 0) break
            total += leidos
            require(total <= limite) { "La respuesta JSON supera el límite permitido." }
            salida.write(buffer, 0, leidos)
        }
        return salida.toByteArray()
    }

    private fun recortarCache(carpeta: File, proteger: File? = null) {
        val todos = carpeta.listFiles()?.filter(File::isFile).orEmpty()
        var total = todos.sumOf(File::length)
        if (total <= MAX_CACHE_BYTES) return

        todos
            .filter { !it.name.endsWith(".part") && it != proteger }
            .sortedBy(File::lastModified)
            .forEach { archivo ->
                if (total <= CACHE_TARGET_BYTES) return@forEach
                val bytes = archivo.length()
                if (archivo.delete()) total -= bytes
            }
    }
