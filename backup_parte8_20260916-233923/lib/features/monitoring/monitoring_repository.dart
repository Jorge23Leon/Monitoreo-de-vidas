...
class MonitoringRepository {
    ...
    try {
      ...
      final results = await Future.wait<dynamic>([
        ...
        _safePaged(
          ...
          query: dataCentralId == null
              ? null
              : {'datacentral': dataCentralId, 'data_central': dataCentralId},
        ),
      ...
      final maps = results[4] as List<Map<String, dynamic>>;
      ...
      for (final source in maps) {
        ...
        if (task != null) {
          ...
          final producer = producerId == null ? null : producerById[producerId];
          final crop = task['crop'];
          ...
          raw['_v3_ranch_name'] = _entityLabel(
            ranch,
            fallback: 'Rancho sin nombre',
          );
          raw['_v3_producer_id'] = producerId;
          raw['_v3_producer_name'] = _entityLabel(
            producer,
            preferred: const ['commercial_name', 'name', 'nombre', 'display_name'],
            fallback: 'Productor sin nombre',
          );
          raw['_v3_crop_id'] = flexibleId(crop) ?? flexibleId(source['crop']);
          raw['_v3_crop_name'] = crop is Map
              ? firstText(Map<String, dynamic>.from(crop), const [
                    'name', 'nombre', 'variety'
                  ])
              : null;
  ...
  Future<List<PhytoCatalogItem>> catalogForCrop(String? cropId) async {
    try {
      final remote = await _paged('api/v1/agro-catalogs/phytosanitary/');
      var items = remote
          .map(PhytoCatalogItem.fromJson)
          .where((e) => e.id.isNotEmpty && !e.isNoPest)
          .toList();

      items = await _hydrateCatalog(items, cropId);
      await _local.cacheCatalog(items);

      final exact = _filterCatalog(items, cropId);
      if (exact.isNotEmpty) return exact;
      return await _local.catalogForCrop(cropId);
    } catch (_) {
      return _local.catalogForCrop(cropId);
    }
  }
  ...
  List<PhytoCatalogItem> _filterCatalog(
    ...
    if (cropId == null || cropId.isEmpty) {
      return items.where((e) => !e.isNoPest).toList(growable: false);
    }
    final exact = items
        .where((e) => !e.isNoPest && e.cropId == cropId)
        .toList(growable: false);
    if (exact.isNotEmpty) return exact;
    return items
        .where((e) => !e.isNoPest && (e.cropId == null || e.cropId!.isEmpty))
        .toList(growable: false);
  }
