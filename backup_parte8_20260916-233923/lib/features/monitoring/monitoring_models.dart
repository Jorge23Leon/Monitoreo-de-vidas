...
class MonitoringHeader {
  ...
  factory MonitoringHeader.fromJson(Map<String, dynamic> json) => MonitoringHeader(
        ...
        cropId: flexibleId(json['_v3_crop_id'] ?? json['crop']),
  ...
  String get plotName => firstText(raw, const [
        '_v3_plot_name', 'plot_name'
      ]) ?? 'Parcela sin nombre';
  String get cropName => firstText(raw, const [
        '_v3_crop_name', 'crop_name'
      ]) ?? 'Cultivo sin nombre';
...
class PhytoCatalogItem {
  ...
  static List<PhytoStage> _extractStages(
    ...
    if (stages.isEmpty) {
      ...
      stages.addAll(
        defaults.map((name) => PhytoStage(name: name, photo: mainPhoto)),
      );
    }
    ...
    for (final stage in stages) {
      unique.putIfAbsent(
        ...
        () => PhytoStage(name: stage.name, photo: stage.photo ?? mainPhoto),
      );
    }
    ...
    for (final stage in stages) {
      ...
      mergedStages.add(
        PhytoStage(
          ...
          photo: detailStage?.photo ?? stage.photo ?? detail.photo ?? photo,
        ),
      );
      seen.add(key);
    }
