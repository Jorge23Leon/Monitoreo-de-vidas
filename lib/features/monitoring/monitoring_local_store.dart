    ...
    for (final item in headers) {
      ...
      if (queued != null && queued.trim().isNotEmpty) {
        ...
        if (existing.isNotEmpty) {
          ...
          final queuedBody = Map<String, dynamic>.from(
            jsonDecode(queued) as Map,
          );

          // Mientras un cambio de estado está pendiente de sincronizar,
          // la versión local manda también para timestamps/motivo. Así una
          // cancelación offline no vuelve a mostrar la fecha vieja al refrescar.
          downloadedRaw.addAll(queuedBody);
          downloadedRaw['status'] =
              queuedBody['status'] ?? existing.first['status'];
          downloadedRaw['additional_notes'] =
              queuedBody.containsKey('additional_notes')
                  ? queuedBody['additional_notes']
                  : oldRaw['additional_notes'];
