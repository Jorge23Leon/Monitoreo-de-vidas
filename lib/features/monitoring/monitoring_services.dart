...
class MonitoringLocationService {
  // Campo: priorizamos la precisión. La app sigue mostrando lecturas hasta
  // 50 m para orientar al técnico, pero solo permite capturar con 25 m o mejor.
  static const double maxCaptureAccuracyMeters = 25;
  static const double preferredCaptureAccuracyMeters = 8;
  static const double maxReadableAccuracyMeters = 50;
  static const Duration maxCaptureAge = Duration(seconds: 10);
  ...
  // Separamos lectura puntual y seguimiento continuo. Tener `timeLimit` en
  // el stream hacía que Android pudiera dejar de actualizar el GPS después de
  // unos segundos. Para campo pedimos la máxima precisión disponible y una
  // frecuencia cercana a la app Kotlin.
  static LocationSettings get _currentSettings => const LocationSettings(
        accuracy: LocationAccuracy.bestForNavigation,
        distanceFilter: 0,
        timeLimit: Duration(seconds: 20),
      );

  static LocationSettings get _streamSettings {
  ...
  static Future<Position> capturePosition() async {
    ...
    // Tomamos varias lecturas reales y conservamos la mejor. Así evitamos
    // guardar la primera lectura del GPS cuando todavía está convergiendo.
    // 8 m o mejor se acepta de inmediato; de lo contrario exigimos 25 m o mejor.
    for (var attempt = 0; attempt < 7; attempt++) {
      try {
        ...
        if (_isSane(candidate, maxAccuracy: 200)) {
          ...
          if (candidate.accuracy <= preferredCaptureAccuracyMeters &&
              age <= maxCaptureAge) {
            return candidate;
          }
        }
    ...
    if (best != null) {
      ...
      if (best.accuracy <= maxCaptureAccuracyMeters) return best;
      throw Exception(
        'La mejor precisión GPS disponible es de ${best.accuracy.toStringAsFixed(0)} m. '
        'Necesitas 25 m o mejor antes de registrar el punto.',
      );
    }
