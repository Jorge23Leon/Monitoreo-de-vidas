...
class MonitoringRepository {
  ...
  Future<void> cancelHeader(String headerId, {required String reason}) async {
    ...
    // Regla de negocio de la app: solo se puede cancelar si todavía no existe
    // ningún hallazgo/captura registrado para la sesión.
    final localRecords = await _local.checkpointsForHeader(headerId);
    ...
    // Si hay red, confirmamos también contra producción. Offline se permite
    // trabajar y el cambio queda en cola, igual que el resto del módulo.
    try {
      final remoteRecords = await _paged(
        'api/v1/monitoring/phyto/checkpoints/',
        query: {'header': headerId},
      );
      if (remoteRecords.isNotEmpty) {
        throw Exception(
          'Este monitoreo ya tiene puntos registrados en CIAGRO y ya no se puede cancelar.',
        );
      }
    } catch (error) {
    ...
    await _queueHeaderPatch(
      headerId,
      status: 'cancelled',
      additionalNotes: cleanReason,
      extra: {'finished_at': finishedAt},
    );
    await _tryQueuedHeaderPatch(headerId);
  }

  Future<bool> autoCloseIfExpired(MonitoringHeader header) async {
    if (header.isCompleted || header.isCancelled) return true;
