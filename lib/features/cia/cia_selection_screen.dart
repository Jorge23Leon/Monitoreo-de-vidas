            ...
            child: FutureBuilder<List<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                ...
                final assignedIds = app.user?.dataCentrals
                        .map((item) => '${item['id'] ?? item['ext_id'] ?? ''}')
                        .where((id) => id.isNotEmpty)
                        .toSet() ??
                    const <String>{};

                // Mismo alcance que el frontend web: para usuarios no admin, las CIAs
                // visibles nacen exclusivamente de `user.datacentrals`, que ya viene
                // filtrado por las asignaciones/permisos del backend. Así nunca se
                // ofrece una CIA que el usuario no tenga asignada aunque el endpoint
                // general llegue a devolver más elementos por caché o compatibilidad.
                ...
                // También podar organizaciones padre sin ninguna CIA hija visible.
                // El árbol queda igual que el frontend: solo se muestran ramas que el
                // usuario realmente puede explorar.
                final visibleParentIds = cias
                    .map(_childParentId)
                    .whereType<String>()
                    .toSet();
                final visibleParents = app.user?.isAdmin == true
                    ? parents
                    : parents
                        .where((item) => visibleParentIds.contains(_id(item)))
                        .toList();
