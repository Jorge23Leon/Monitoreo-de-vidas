    ...
    try {
      ...
      for (final source in maps) {
        ...
        if (task != null) {
          ...
          final cropId = flexibleId(crop) ?? flexibleId(source['crop']);
          ...
          final embeddedCrop = crop is Map
              ? Map<String, dynamic>.from(crop)
              : null;
          raw['_v3_crop_name'] =
              (embeddedCrop == null
                  ? null
                  : firstText(embeddedCrop, const ['name', 'nombre', 'variety'])) ??
              (cropCatalog == null
                  ? null
          ...
          final cropPhoto = photoUrlFrom(embeddedCrop) ??
              photoUrlFrom(cropCatalog) ??
              photoUrlFrom(source['crop']);
          if (cropPhoto != null) raw['_v3_crop_photo'] = cropPhoto;
