...
class MonitoringLocationService {
  ...
  static Future<Position> capturePosition() async {
    ...
    // Tomamos varias lecturas reales. Esto evita guardar la primera lectura
    // disponible si el GPS todavía está convergiendo. 12 m o mejor se acepta
    // de inmediato; si no, se conserva la mejor siempre que llegue a 35 m.
    for (var attempt = 0; attempt < 5; attempt++) {
      try {
        ...
        if (_isSane(candidate, maxAccuracy: 200)) {
          ...
          if (candidate.accuracy <= 12 && age <= maxCaptureAge) {
            return candidate;
          }
        }
      } catch (error) {
